...
router.get('/product', async (req, res, next) => {
  const sql = `
    SELECT product.*, vendor.business_name FROM product
    JOIN vendor ON product.vendor_id = vendor.id WHERE product.id IN ( SELECT customer_order_detail.product_id FROM customer_order
